...
studentImportRouter.post('/:batchId/approve', async (req, res, next) => {
  ...
  try {
    ...
    const [bad]: any = await conn.query(`SELECT COUNT(*) n FROM import_student_rows r WHERE r.batch_id=? AND (r.source_nis IS NULL OR TRIM(r.source_nis)='' OR TRIM(r.source_name)='' OR r.source_class IS NULL OR TRIM(r.source_class)='' OR r.match_status IN('AMBIGUOUS','EXACT_NAME_REVIEW','REVIEW','RESOLVED_HISTORICAL') OR (SELECT COUNT(*) FROM classes c WHERE UPPER(TRIM(c.code))=UPPER(TRIM(r.source_class)) AND c.active=1)<>1)`, [b.data]);
...
studentImportRouter.post('/:batchId/apply', async (req, res, next) => {
  ...
  try {
    ...
    const [rows]: any = await conn.query(`SELECT id,source_nis,source_name,source_class,birth_place,birth_date,matched_student_id,match_status FROM import_student_rows WHERE batch_id=? ORDER BY source_row_number FOR UPDATE`, [b.data]);
    ...
    for (const r of rows) {
      ...
      const nis = String(r.source_nis ?? '').trim(), name = String(r.source_name ?? '').trim(), sourceClass = String(r.source_class ?? '').trim();
